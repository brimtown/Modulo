...
public class Oscillator {
	...
	public Oscillator() {
		// 440hz = "concert A", standard pitch
		frequency = 440;
		volume = 50;
		isPlaying = false;
		...
		// Pre-generates the lookup tables for each waveform, allows instant
		// waveform switching
		sqr = generateSqr();
		sin = generateSin();
		tri = generateTri();
	...
	public void pause() {
		// Checks if audioTrack is initialized and if so, immediately pauses and
	...
	public AudioTrack initializeAudioTrack() {
		...
		Float scaled = (float)(volume/100.0f);
		returnTrack.setStereoVolume(AudioTrack.getMaxVolume() * scaled, AudioTrack.getMaxVolume() * scaled);
	...
	public void setVolume(int vol) {
		volume = vol;
		Float scaled = (float) (volume / 100.0f);
		audioTrack.setStereoVolume(AudioTrack.getMaxVolume() * scaled, AudioTrack.getMaxVolume() * scaled);
	}
	...
	private short[] generateSqr() {
		short[] returnArray = new short[sampleRate];
		// The square wave is represented by two values (Short.MAX_VALUE and
		// Short.MIN_VALUE)
		// with the max value for the first half of the waveform, and the min
		// for the second
		for (int i = 0; i < sampleRate; i++) {
			if (i < (sampleRate / 2)) {
	...
	public void fillBuffer() {
		for (int i = 0; i < buffer.length; i++) {
			...
			// Sets the short at buffer[i] to the value found in the lookup
			// table
			// of the current waveform sample
			buffer[i] = sample[phase];

			// Increments the phase by the current frequency, allowing it to
			// step
			// through the table at the correct speed and resulting in a change
			// of pitch
			phase += frequency;
		}
